...
func TestSandboxClaimVolumeClaimTemplatesRoundTrip(t *testing.T) {
	...
	if _, ok := alpha.Annotations[v1beta1SandboxClaimVolumeClaimTemplatesAnnotation]; !ok {
	...
	gotJSON, _ := json.Marshal(dst.Spec.VolumeClaimTemplates)
	wantJSON, _ := json.Marshal(vcts)
	if string(gotJSON) != string(wantJSON) {
		t.Fatalf("volumeClaimTemplates not preserved:\n got %s\nwant %s", gotJSON, wantJSON)
	}
	if _, ok := dst.Annotations[v1beta1SandboxClaimVolumeClaimTemplatesAnnotation]; ok {
...
func TestSandboxClaimNoVolumeClaimTemplatesNoAnnotation(t *testing.T) {
	...
	if _, ok := alpha.Annotations[v1beta1SandboxClaimVolumeClaimTemplatesAnnotation]; ok {
		t.Fatalf("annotation set when volumeClaimTemplates is nil")
	}
}
